...
Currently only supports Windows, other systems are not supported
...
## GUI Tool
If you're not used to commands, you can download the GUI version, file naming is roughly `4nxci-v4.03_GUI.zip`
...
-r, --rename             Set exported nsp file name
--keepncaid              Keep current ncas ID 
```

## About keyset File
Copy the following code into a txt file, then rename it to `keys.dat` 
Import using GUI or command line tool
