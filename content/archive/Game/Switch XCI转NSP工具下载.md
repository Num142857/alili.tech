...
## 4NXCI 是将 XCI 文件转换为 NSP 的工具

目前只支持windows,其他系统不支持
...
## GUI工具
命令用不习惯的可以下载GUI的版本,文件命名大致为`4nxci-v4.03_GUI.zip
`

![图 1](https://incomparable9527.coding.net/p/imageBed/d/imageBed/git/raw/master/4a89dd91123467dc70d9633e66182469cf9c5e2d0a555bc62b0a379a3f41da8b.png)  
...
Options:  
-k, --keyset             设置keyset文件路径, 默认为 ./keys.dat  
-h, --help               显示帮助信息
-t, --tempdir            设置临时文件夹
-o, --outdir             设置导出文件夹
-r, --rename             设置导出的nsp文件的文件名
--keepncaid              保持当前ncas ID 
```

## 关于keyset 文件
拷贝以下代码放到一个txt文件中,然后重命名为 `keys.dat` 
使用GUI或者命令行工具导入就可以了
